...
class FetchGroup {
  ...
  *     method can be used to ensure that `grandChild` meets those requirement.
  ...
  toPlanNode(
    ...
    operation = operation.optimize(fragments?.forSubgraph(this.subgraphName, subgraphSchema,), {minUsagesToOptimize: queryPlannerConfig.experimental.minFragmentUsagesToOptimize, autoFragmentize: true});
    operation = operation.autoFragmentize(operation.fragments ? operation.fragments : new NamedFragments() );
